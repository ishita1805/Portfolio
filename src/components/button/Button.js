...
const Button = (props) => {
    ...
    if(props.internal===0) return (
        <div>
            {
                props.icon?
                <Link to={props.id} spy={true} offset={-58} smooth={true} duration={500}><button className={props.fill?`filled-button-i ${props.color}`:`line-button ${props.color}`}><i className={`ib ${props.label}`}></i></button></Link>
        ...
        <div>
            {
                props.icon?
                <a href={props.id} target='__blank'><button className={props.fill?`filled-button-i ${props.color}`:`line-button ${props.color}`}><i className={`ib ${props.label}`}></i></button></a>:
